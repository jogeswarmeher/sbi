...
st.title("💬 Chat with your Document (PDF / PNG / JPG)")
...
# ---------------- FILE → IMAGES ----------------
def load_images_from_file(file_path: str, file_suffix: str, dpi: int = 300) -> List[Image.Image]:
    """
    Converts PDF or Image file to list of PIL Images
    """
    images = []

    # If PDF → render each page
    ...
    # If image → open directly
    elif file_suffix.lower() in [".png", ".jpg", ".jpeg"]:
...
# ---------------- OCR ----------------
def extract_text_from_file(file_path: str, file_suffix: str) -> List[str]:
    images = load_images_from_file(file_path, file_suffix)
...
if uploaded_file:
    ...
    with st.spinner("🔍 Extracting text using Surya OCR..."):
        extracted_pages = extract_text_from_file(file_path, suffix)
